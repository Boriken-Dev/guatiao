...
//! A C program that loads and dumps a value in three formats.
//!
//! Everything else in this crate's suite drives the API from Rust. This
//! one does not: it compiles C against the committed header, links the
//! artifact, and runs it — which is the only way to find out whether the
//! exported surface is actually usable by the audience it exists for.
//!
//! # Skipping
//!
//! With no C compiler, or with the shared libraries not built, this
//! prints a stated skip and passes. A missing toolchain is an environment
//! fact; failing over it would make the suite red where nothing is wrong.
//! The header-drift check below never skips.
...
/// header describing a different ABI — and the consumer who finds out is
/// somebody else's.
///
/// Never skips.
#[test]
...
fn the_committed_header_is_what_this_build_rendered() {
    ...
    let committed = std::fs::read_to_string(manifest.join("include/guatiao_serde.h"))
        .expect("the header is committed");
    let fresh = std::fs::read_to_string(env!("GUATIAO_SERDE_GENERATED_HEADER"))
    ...
    assert_eq!(
        ...
        "the committed header is not what this build renders. Regenerate it:\n  \
         GUATIAO_WRITE_HEADER=1 cargo build -p guatiao-serde \
         --features c-header,json,toml,yaml"
    );
}
...
/// A C program loads and dumps a value in three formats.
#[test]
fn a_c_program_loads_and_dumps_a_value() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let source = manifest.join("tests/c_consumer/serde_consumer.c");
    assert!(
        source.exists(),
    ...
    let (Some((dir, ours)), Some((_, core))) =
        (shared_library("guatiao_serde"), shared_library("guatiao"))
    else {
        println!(
    ...
    let out = dir.join(if cfg!(windows) {
        "guatiao_serde_consumer.exe"
    } else {
        "guatiao_serde_consumer"
    });
